...
class Chained():
    ...
    def insert_last(self, pop_breed, pop_color):
        ...
        for item in collection:
            for obj in item:
                pop_id = int(obj) + 1
        stm = insert(DbModel).values(id=pop_id, breed=pop_breed,color=pop_color)
        return self.commit_refresh(stm)

    def insert_entry(self, pop_breed, pop_color):
        self.insert_last(pop_breed, pop_color)
        return self.all()

    def update_entry(self, pop_id, pop_breed, pop_color):
        stm = update(DbModel) \
            .where(DbModel.id == pop_id) \
            .values(breed=pop_breed,color=pop_color)
        return self.commit_refresh(stm)
